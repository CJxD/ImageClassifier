...
public class BoVW {
	...
	public static void main(String[] args) throws FileSystemException {
		if (args.length < 2) throw new IllegalArgumentException("Usage: BoVW <training uri> <testing uri>");
		
		File trainingFile = new File(args[0]);
		File testingFile = new File(args[1]);
		
		VFSGroupDataset<FImage> training = new VFSGroupDataset<>(trainingFile.getAbsolutePath(), ImageUtilities.FIMAGE_READER);
		VFSListDataset<FImage> testing = new VFSListDataset<>(testingFile.getAbsolutePath(), ImageUtilities.FIMAGE_READER);

		BoVW bovw = new BoVW();
		
		// Hack to fix the Java inheritance of VFSListDataset
		bovw.train(GroupedUniformRandomisedSampler.sample(training, 30));
		
		Map<FImage, List<ScoredAnnotation<String>>> results = bovw.classify(testing);
		
		for (Entry<FImage, List<ScoredAnnotation<String>>> e : results.entrySet()) {
	...
	public Map<FImage, List<ScoredAnnotation<String>>> classify(ListDataset<FImage> testData) {
		if (quantiser == null) throw new IllegalStateException("Classifier is not trained");
		if (annotator == null) throw new IllegalStateException("Annotator is not trained");
	...
	protected void trainAnnotator(GroupedDataset<String, ListDataset<FImage>, FImage> trainingData) {
		if (quantiser == null) throw new IllegalStateException("Quantiser is not trained");
	...
	/**
	 * Segments an image into centre-meaned, normalised patches.
	 * Will crop any pixels from the right and bottom of the image that do not
	 * divide into {@link #patchSeparation}.
	 * @param image
